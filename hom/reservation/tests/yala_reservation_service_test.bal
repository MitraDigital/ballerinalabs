...
@test:Config
function testYalaReservationServiceSuccess() {
    ...
    var response = yalaReservationServiceEP -> execute("POST", "/reservation", request);
    match response {
        http:Response res => {
...
@test:Config
function testYalaReservationServiceInvalidDataType() {
    http:Request request = new;
    json reservationRequest = {
        "customerId": "11111111v",
        "customerName" : "Randima",
